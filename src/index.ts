...
const isTelegramError = (error: any): error is TTelegramError => error && error.response && error.response.body && typeof error.response.body.error_code === 'number';
...
const handleSendError = (error: any, targetChatId: string) => {
    if (isTelegramError(error)) {
        switch (error.response?.body?.error_code) {
            case 403:
                console.log(`Bot is blocked by the user or doesn't have access to chat with ID ${targetChatId}`);
                break;
            case 404:
            ...
            case 429:
                ...
                break;
            default:
                console.error(`Error sending message to chat ${targetChatId}: ${error.message}`);
        }
    } else {
...
const createTgNotificationBot = ({ token, chatId }: TgNotificationBot) => {
    ...
    const normalizeChatId = async (chatIdToNormalize: ChatId): Promise<string> => {
        if (typeof chatIdToNormalize === 'number') return chatIdToNormalize.toString();
        if (chatIdToNormalize.startsWith('-100') || chatIdToNormalize.startsWith('-')) return chatIdToNormalize;
